export const mockProducts = [
  {
    id: 1,
    name: "Elegant Floral Kurti",
    price: 1299,
    originalPrice: 1899,
    discount: 32,
    category: "kurtis",
    images: [
      "https://images.pexels.com/photos/33133599/pexels-photo-33133599.jpeg",
      "https://images.pexels.com/photos/20777203/pexels-photo-20777203.jpeg",
      "https://images.pexels.com/photos/3061814/pexels-photo-3061814.jpeg",
      "https://images.pexels.com/photos/1040945/pexels-photo-1040945.jpeg"
    ],
    sizes: ["S", "M", "L", "XL"],
    colors: ["Blue", "Pink", "White", "Yellow"],
    rating: 4.5,
    reviews: 128,
    stock: 8,
    isNew: false,
    isTrending: true,
    sustainable: true,
    description: "Beautiful floral printed kurti perfect for casual and semi-formal occasions. Made with premium organic cotton fabric for ultimate comfort and style. Features traditional block printing techniques that support local artisans.",
    fabric: "100% Organic Cotton",
    care: "Machine wash cold, tumble dry low",
    createdAt: "2024-01-15",
    deliveryTime: "2-4 business days",
    returnPolicy: "30-day easy returns",
    highlights: [
      "Organic cotton fabric",
      "Hand block printed",
      "Supports local artisans",
      "Breathable and comfortable",
      "Perfect for all seasons"
    ]
  },
  {
    id: 2,
    name: "Chic Designer Dress",
    price: 2199,
    originalPrice: 2999,
    discount: 27,
    category: "dresses",
    images: [
      "https://images.pexels.com/photos/33161433/pexels-photo-33161433.jpeg",
      "https://images.pexels.com/photos/33167324/pexels-photo-33167324.jpeg",
      "https://images.pexels.com/photos/33171268/pexels-photo-33171268.jpeg",
      "https://images.pexels.com/photos/1021693/pexels-photo-1021693.jpeg"
    ],
    sizes: ["XS", "S", "M", "L", "XL"],
    colors: ["Black", "Navy", "Maroon", "Emerald", "Wine"],
    rating: 4.7,
    reviews: 95,
    stock: 15,
    isNew: true,
    isTrending: false,
    sustainable: false,
    description: "Elegant designer dress with modern cut and premium fabric. Perfect for evening events and special occasions. Features contemporary silhouette with attention to detail.",
    fabric: "Premium Polyester Blend",
    care: "Dry clean only",
    createdAt: "2024-01-14",
    deliveryTime: "1-3 business days",
    returnPolicy: "30-day easy returns",
    highlights: [
      "Designer cut",
      "Premium fabric",
      "Perfect for occasions",
      "Modern silhouette",
      "Available in 5 colors"
    ]
  },
  {
    id: 3,
    name: "Stylish Casual Top",
    price: 899,
    originalPrice: 1299,
    discount: 31,
    category: "tops",
    images: [
      "https://images.pexels.com/photos/3061814/pexels-photo-3061814.jpeg",
      "https://images.pexels.com/photos/33171268/pexels-photo-33171268.jpeg",
      "https://images.pexels.com/photos/33167324/pexels-photo-33167324.jpeg"
    ],
    sizes: ["S", "M", "L"],
    colors: ["White", "Black", "Pink", "Blue"],
    rating: 4.3,
    reviews: 67,
    stock: 3,
    isNew: false,
    isTrending: false,
    description: "Trendy casual top with modern design. Pairs perfectly with high-waisted jeans or skirts.",
    fabric: "Cotton Blend",
    care: "Machine wash warm",
    createdAt: "2024-01-13"
  },
  {
    id: 4,
    name: "Designer Ethnic Anarkali",
    price: 3499,
    originalPrice: 4999,
    discount: 30,
    category: "ethnic",
    images: [
      "https://images.pexels.com/photos/20777203/pexels-photo-20777203.jpeg",
      "https://images.pexels.com/photos/33133599/pexels-photo-33133599.jpeg",
      "https://images.pexels.com/photos/3061814/pexels-photo-3061814.jpeg"
    ],
    sizes: ["S", "M", "L", "XL", "XXL"],
    colors: ["Royal Blue", "Red", "Green"],
    rating: 4.8,
    reviews: 156,
    stock: 12,
    isNew: false,
    isTrending: true,
    description: "Stunning Anarkali suit with intricate embroidery and modern silhouette. Perfect for weddings and festivals.",
    fabric: "Georgette with Embroidery",
    care: "Dry clean only",
    createdAt: "2024-01-12"
  },
  {
    id: 5,
    name: "Modern Casual Jacket",
    price: 1899,
    originalPrice: 2499,
    discount: 24,
    category: "jackets",
    images: [
      "https://images.pexels.com/photos/33167324/pexels-photo-33167324.jpeg",
      "https://images.pexels.com/photos/33161433/pexels-photo-33161433.jpeg",
      "https://images.pexels.com/photos/33171268/pexels-photo-33171268.jpeg"
    ],
    sizes: ["S", "M", "L", "XL"],
    colors: ["Light Blue", "Dark Blue", "Black"],
    rating: 4.4,
    reviews: 89,
    stock: 7,
    isNew: true,
    isTrending: false,
    description: "Classic modern jacket with contemporary styling. A wardrobe essential for casual and semi-formal looks.",
    fabric: "Cotton Denim",
    care: "Machine wash cold",
    createdAt: "2024-01-11"
  },
  {
    id: 6,
    name: "Bohemian Style Dress",
    price: 1599,
    originalPrice: 2199,
    discount: 27,
    category: "dresses",
    images: [
      "https://images.pexels.com/photos/33171268/pexels-photo-33171268.jpeg",
      "https://images.pexels.com/photos/3061814/pexels-photo-3061814.jpeg",
      "https://images.pexels.com/photos/33133599/pexels-photo-33133599.jpeg"
    ],
    sizes: ["S", "M", "L", "XL"],
    colors: ["Multicolor", "Beige", "Black"],
    rating: 4.2,
    reviews: 73,
    stock: 9,
    isNew: false,
    isTrending: false,
    description: "Flowy bohemian dress with beautiful prints and comfortable fit. Perfect for summer days and casual outings.",
    fabric: "Rayon",
    care: "Hand wash recommended",
    createdAt: "2024-01-10"
  },
  {
    id: 7,
    name: "Formal Blazer Coat",
    price: 2899,
    originalPrice: 3999,
    discount: 28,
    category: "blazers",
    images: [
      "https://images.pexels.com/photos/33161433/pexels-photo-33161433.jpeg",
      "https://images.pexels.com/photos/33167324/pexels-photo-33167324.jpeg",
      "https://images.pexels.com/photos/20777203/pexels-photo-20777203.jpeg"
    ],
    sizes: ["S", "M", "L", "XL"],
    colors: ["Black", "Navy", "Grey"],
    rating: 4.6,
    reviews: 112,
    stock: 5,
    isNew: false,
    isTrending: true,
    description: "Sophisticated blazer for professional and formal occasions. Tailored fit with premium finish and modern design.",
    fabric: "Polyester Suiting",
    care: "Dry clean only",
    createdAt: "2024-01-09"
  },
  {
    id: 8,
    name: "Summer Cotton Kurti",
    price: 1199,
    originalPrice: 1599,
    discount: 25,
    category: "kurtis",
    images: [
      "https://images.pexels.com/photos/3061814/pexels-photo-3061814.jpeg",
      "https://images.pexels.com/photos/33133599/pexels-photo-33133599.jpeg",
      "https://images.pexels.com/photos/20777203/pexels-photo-20777203.jpeg"
    ],
    sizes: ["S", "M", "L", "XL"],
    colors: ["White", "Pink", "Blue", "Yellow"],
    rating: 4.5,
    reviews: 94,
    stock: 11,
    isNew: true,
    isTrending: false,
    description: "Comfortable cotton kurti perfect for daily wear. Features beautiful patterns and breathable fabric for all-day comfort.",
    fabric: "100% Cotton",
    care: "Machine wash cold",
    createdAt: "2024-01-08"
  },
  {
    id: 9,
    name: "Party Wear Gown",
    price: 3299,
    originalPrice: 4499,
    discount: 27,
    category: "gowns",
    images: [
      "https://images.pexels.com/photos/33171268/pexels-photo-33171268.jpeg",
      "https://images.pexels.com/photos/33161433/pexels-photo-33161433.jpeg",
      "https://images.pexels.com/photos/33167324/pexels-photo-33167324.jpeg"
    ],
    sizes: ["XS", "S", "M", "L", "XL"],
    colors: ["Red", "Black", "Gold", "Silver"],
    rating: 4.9,
    reviews: 167,
    stock: 4,
    isNew: true,
    isTrending: true,
    description: "Elegant party wear gown with stunning embellishments. Perfect for special occasions and evening events.",
    fabric: "Silk with Embellishments",
    care: "Dry clean only",
    createdAt: "2024-01-07"
  },
  {
    id: 10,
    name: "Traditional Salwar Suit",
    price: 2199,
    originalPrice: 2999,
    discount: 27,
    category: "ethnic",
    images: [
      "https://images.pexels.com/photos/20777203/pexels-photo-20777203.jpeg",
      "https://images.pexels.com/photos/33133599/pexels-photo-33133599.jpeg",
      "https://images.pexels.com/photos/3061814/pexels-photo-3061814.jpeg"
    ],
    sizes: ["S", "M", "L", "XL", "XXL"],
    colors: ["Green", "Pink", "Blue", "Orange"],
    rating: 4.6,
    reviews: 103,
    stock: 8,
    isNew: false,
    isTrending: false,
    description: "Beautiful traditional salwar suit with contemporary styling. Perfect for festivals and cultural events.",
    fabric: "Cotton with Thread Work",
    care: "Machine wash gentle",
    createdAt: "2024-01-06"
  }
];

export const categories = [
  {
    id: 1,
    name: "Kurtis",
    slug: "kurtis",
    image: "https://images.pexels.com/photos/33133599/pexels-photo-33133599.jpeg",
    count: 45
  },
  {
    id: 2,
    name: "Dresses",
    slug: "dresses",
    image: "https://images.pexels.com/photos/33161433/pexels-photo-33161433.jpeg",
    count: 32
  },
  {
    id: 3,
    name: "Ethnic Wear",
    slug: "ethnic",
    image: "https://images.pexels.com/photos/20777203/pexels-photo-20777203.jpeg",
    count: 28
  },
  {
    id: 4,
    name: "Tops",
    slug: "tops",
    image: "https://images.pexels.com/photos/3061814/pexels-photo-3061814.jpeg",
    count: 56
  },
  {
    id: 5,
    name: "Gowns",
    slug: "gowns",
    image: "https://images.pexels.com/photos/33171268/pexels-photo-33171268.jpeg",
    count: 24
  }
];
